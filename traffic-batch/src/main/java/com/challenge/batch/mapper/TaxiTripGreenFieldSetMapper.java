...
@Component
public class TaxiTripGreenFieldSetMapper implements FieldSetMapper<TaxiTrip> {

    @Override
    public TaxiTrip mapFieldSet(FieldSet fieldSet) {
        final TaxiTrip taxiTrip = new TaxiTrip(
                null,
                Timestamp.from(fieldSet.readDate("lpep_pickup_datetime").toInstant()),
                Timestamp.from(fieldSet.readDate("lpep_dropoff_datetime").toInstant()),
